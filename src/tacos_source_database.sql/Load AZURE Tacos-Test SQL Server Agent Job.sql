...
/****** Object:  Job [Load AZURE Tacos-Test]    Script Date: 2/11/2021 1:53:46 PM ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [[Uncategorized (Local)]]    Script Date: 2/11/2021 1:53:46 PM ******/
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'Load AZURE Tacos-Test', 
		@enabled=1, 
		...
		@notify_level_page=0, 
		@delete_level=0, 
		@description=N'Truncate and reload the local tables, and then truncate and reload the Courses table on Azure Tacos-Test.', 
		@category_name=N'[Uncategorized (Local)]', 
		@owner_login_name=N'sa', 
		@notify_email_operator_name=N'apprequests', @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Make a local copy of BIA DESII_Courses table]    Script Date: 2/11/2021 1:53:47 PM ******/
 ...
 UPDATE [dbo].[DESII_Courses]
  ...
  WHERE AcademicYear = ''2017-18'' AND 
	...
	AcademicTermCode = 201609', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Make a local copy of CourseDescription table]    Script Date: 2/11/2021 1:53:47 PM ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Make a local copy of CourseDescription table', 
		...
		@os_run_priority=0, @subsystem=N'TSQL', 
		@command=N'
-- Truncate the local CourseDescription table:
...
', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Copy DESII Course data for the last two full academic years.]    Script Date: 2/11/2021 1:53:47 PM ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Copy DESII Course data for the last two full academic years.', 
		...
		@retry_interval=0, 
		@os_run_priority=0, @subsystem=N'TSQL', 
		@command=N'-- Truncate DESII_CoursesForLastSixQuarters table: 
...
', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Truncate and repopulate the AggregatedCourses table courses]    Script Date: 2/11/2021 1:53:47 PM ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Truncate and repopulate the AggregatedCourses table courses', 
		@step_id=4, 
		@cmdexec_success_code=0, 
		@on_success_action=3, 
		@on_success_step_id=0, 
...
-- the non-cross-listed courses:
	...
	SELECT * FROM [dbo].[udf_GetCombinedDataForAllCourses]()', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Truncate and reload Courses table with aggregated data, plus other active courses from course catalog:]    Script Date: 2/11/2021 1:53:47 PM ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Truncate and reload Courses table with aggregated data, plus other active courses from course catalog:', 
		...
		@retry_interval=0, 
		@os_run_priority=0, @subsystem=N'TSQL', 
		@command=N'-- Truncate local Courses table:
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Update Data For Cross-listed Courses', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Truncate and Reload AZURE Courses]    Script Date: 2/11/2021 1:53:47 PM ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Truncate and Reload AZURE Courses', 
		@step_id=8, 
		@cmdexec_success_code=0, 
		@on_success_action=3, 
		@on_success_step_id=0, 
	...
	EXECUTE [AZURE_TACOS_TEST].[tacos-test].dbo.sp_executesql @statement = N''TRUNCATE TABLE dbo.Courses''

-- Reload with updated data:
INSERT INTO [AZURE_TACOS_TEST].[tacos-test].dbo.Courses (
       [Number]
      ,[AverageEnrollment]
      ,[AverageSectionsPerCourse]
      ,[Name]
...
', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Truncate and Reload AZURE CourseDescription table]    Script Date: 2/11/2021 1:53:47 PM ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Truncate and Reload AZURE CourseDescription table', 
	...
	EXECUTE AZURE_TACOS_TEST.[tacos-test].dbo.sp_executesql @statement = N''TRUNCATE TABLE dbo.CourseDescription''

-- Reload with updated table:
  INSERT INTO [AZURE_TACOS_TEST].[Tacos-test].[dbo].[CourseDescription] (
       [Course]
      ,[SubjectCode]
      ,[CourseNumber]
      ,[CrossListing]
...
', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Truncate and Reload AZURE DESII_Courses table with data for all terms within the past four academic years]    Script Date: 2/11/2021 1:53:47 PM ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Truncate and Reload AZURE DESII_Courses table with data for all terms within the past four academic years', 
	...
	EXECUTE AZURE_TACOS_TEST.[Tacos-test].dbo.sp_executesql @statement = N''TRUNCATE TABLE dbo.DESII_Courses'' 
...
-- for displaying further information regarding course enrollment.
 ...
 INSERT INTO [AZURE_TACOS_TEST].[Tacos-test].[dbo].[DESII_Courses](
       [AcademicYear]
      ,[AcademicTermCode]
      ,[College]
      ,[DeptName]
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'Run AZURE Tacos-test Weekly Update', 
		@enabled=0, 
		@freq_type=1, 
		@freq_interval=0, 
		@freq_subday_type=0, 
