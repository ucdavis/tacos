...
/****** Object:  UserDefinedFunction [dbo].[udf_GetCombinedDataForNonCrossListedCourses]    Script Date: 10/19/2018 3:34:35 PM ******/
SET ANSI_NULLS ON
GO
...
--	all the non cross-listed courses for the past two academic years.
--
...
--
-- =============================================
CREATE FUNCTION [dbo].[udf_GetCombinedDataForNonCrossListedCourses] 
()
RETURNS 
@NonCrossListedCourses TABLE 
(
...
AS
BEGIN
	INSERT INTO @NonCrossListedCourses
	SELECT DISTINCT   t1.SubjectCode, t1.CourseNumber, NULL AS DeptName, t1.SubjectCode + t1.CourseNumber AS Number,
		...
		SUM(CONVERT(float, t1.NumCreditSections)) / 2  AS TimesOfferedPerYear, 0 AS IsCrossListed
	...
	LEFT OUTER JOIN 
	(
		SELECT DISTINCT Course, CreatedOn, Title, CrossListing
		FROM CourseDescription 
		WHERE Status = 'Active'
	) t4 ON t5.Course = t4.Course AND t5.CreatedOn = t4.CreatedOn
	WHERE CrossListing IS NULL
