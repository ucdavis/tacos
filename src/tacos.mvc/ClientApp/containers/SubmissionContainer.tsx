...
export default class SubmissionContainer extends React.Component<IProps, IState> {
    ...
    return (
      <div className="tacos-page-with-summary">
        <div className="tacos-action-row tacos-action-row--end">
          <button className="btn btn-primary gap-2" onClick={this.onAddRequest} type="button">
            Create New Request
  ...
  private submissionReaderTotal = () => {
    ...
    return requests
      .filter((r) => !r.isDeleted)
      .reduce((acc, req) => {
        const value = req.exception
          ? req.exceptionAnnualizedReaderTotal
          : req.annualizedReaderTotal;
        return acc + value;
      }, 0);
  };
    ...
    try {
      ...
      this.setState((state) => {
        ...
        const request = {
          ...state.requests[requestIndex],
          calculationError:
            "Unable to refresh calculated totals. Save or submit will recalculate on the server.",
          isRecalculating: false,
        };
